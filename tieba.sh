...
#load the config
USERNAME=`cat ${DIR1}conf_tieba.txt | grep 'username' | awk -F '=' '{print $2}'`
PASSWORD=`cat ${DIR1}conf_tieba.txt | grep 'password' | awk -F '=' '{print $2}'`
...
while [ 1 -eq 1 ]; do
	...
	for TIEBANAME in `cat conf_tieba_list.txt`; do
		...
		row=`wc -l ${DIR1}conf_black_title.txt | awk '{print $1}'`
		if [ $row -eq 0 ]; then
			echo 'Nothing in the black_title .'
		fi

		for black_title in `cat ${DIR1}conf_black_title.txt`; do
			for index in {1..50}; do
